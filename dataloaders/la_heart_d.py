...
class CenterCrop(object):
    ...
    def __call__(self, sample):
        ...
        if label.shape[0] <= self.output_size[0] or label.shape[1] <= self.output_size[1] or label.shape[2] <= \
            ...
            image = np.pad(image, [(ph, ph), (pw, pw), (ph, ph)],
                           mode='constant', constant_values=0)
            label = np.pad(label, [(ph, ph), (pw, pw), (ph, ph)],
                           mode='constant', constant_values=0)
        ...
        label = label[d1:d1 + self.output_size[0], w1:w1 +
                      self.output_size[1], h1:h1 + self.output_size[2]]
        image = image[d1:d1 + self.output_size[0], w1:w1 +
                      self.output_size[1], h1:h1 + self.output_size[2]]
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        if label.shape[0] <= self.output_size[0] or label.shape[1] <= self.output_size[1] or label.shape[2] <= \
            ...
            pw = max((self.output_size[1] - label.shape[1]) // 2 + 3, 0)
            ph = max((self.output_size[2] - label.shape[2]) // 2 + 3, 0)
            image = np.pad(image, [(pd, pd), (pw, pw), (ph, ph)],
                           mode='constant', constant_values=0)
            label = np.pad(label, [(pd, pd), (pw, pw), (ph, ph)],
                           mode='constant', constant_values=0)
            if self.with_sdf:
                sdf = np.pad(sdf, [(pd, pd), (pw, pw), (ph, ph)],
                             mode='constant', constant_values=0)
        ...
        image = image[d1:d1 + self.output_size[0], w1:w1 +
                      self.output_size[1], h1:h1 + self.output_size[2]]
        if self.with_sdf:
            sdf = sdf[d1:d1 + self.output_size[0], w1:w1 +
                      self.output_size[1], h1:h1 + self.output_size[2]]
